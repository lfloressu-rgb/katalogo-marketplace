...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch('http://localhost:5000/api/admin/metricas', { headers }).then(r => r.json()),
      fetch('http://localhost:5000/api/admin/tiendas', { headers }).then(r => r.json()),
      fetch('http://localhost:5000/api/admin/usuarios', { headers }).then(r => r.json()),
      fetch('http://localhost:5000/api/admin/categorias', { headers }).then(r => r.json()),
      fetch('http://localhost:5000/api/admin/finanzas', { headers }).then(r => r.json()),
      fetch('http://localhost:5000/api/admin/ordenes', { headers }).then(r => r.json())
  ...
  const loadFinanzas = async () => {
    ...
    try {
      const [finRes, liqRes, confRes] = await Promise.all([
        fetch(`http://localhost:5000/api/admin/finanzas?rango=${rangoFecha}`, { headers }),
        fetch('http://localhost:5000/api/admin/liquidaciones', { headers }),
        fetch('http://localhost:5000/api/admin/config', { headers })
      ]);
  ...
  const handlePagarLiquidacion = async (tiendaId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`http://localhost:5000/api/admin/liquidaciones/${tiendaId}/pagar`, {
        method: 'PUT',
  ...
  const handleAprobar = async (id) => {
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`http://localhost:5000/api/admin/tiendas/${id}/aprobar`, {
        method: 'PUT',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const handleRechazar = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`http://localhost:5000/api/admin/tiendas/${id}/rechazar`, {
        method: 'PUT',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const handleCrearCategoria = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setNuevaCategoria('');
        // Recargar categorías
        const catRes = await fetch('http://localhost:5000/api/admin/categorias', { headers: { 'Authorization': `Bearer ${token}` } });
