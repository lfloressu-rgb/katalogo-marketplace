...
export default function VendedorPage() {
  ...
  const loadData = () => {
    ...
    Promise.all([
      fetch('http://localhost:5000/api/vendedor/tienda', { headers }).then(r => r.json()),
      fetch('http://localhost:5000/api/vendedor/productos', { headers }).then(r => r.json()),
      fetch('http://localhost:5000/api/vendedor/ordenes', { headers }).then(r => r.json()),
      fetch('http://localhost:5000/api/vendedor/reportes/ventas', { headers }).then(r => r.json()),
      fetch('http://localhost:5000/api/vendedor/cupones', { headers }).then(r => r.json()),
      fetch('http://localhost:5000/api/soporte/vendedor', { headers }).then(r => r.json()),
  ...
  const handlePublicarProducto = async () => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:5000/api/vendedor/productos', {
        method: 'POST',
  ...
  const handleCrearCupon = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch('http://localhost:5000/api/vendedor/cupones', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('token')}` },
  ...
  const handleGuardarEdicion = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/vendedor/productos/${productoEditando.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('token')}` },
        body: JSON.stringify(payload)
      });
  ...
  const handleResolverReclamo = async (id) => {
    try {
      await fetch(`http://localhost:5000/api/soporte/vendedor/${id}/resolver`, {
        method: 'PUT',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
      loadData();
